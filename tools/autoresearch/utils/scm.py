...
provides commit/goto/current_commit operations. The autoresearch system
...
def detect_scm(source_dir: str) -> str:
...
def current_commit(scm: str, source_dir: str) -> str:
...
def commit(scm: str, source_dir: str, message: str) -> str:
    ...
    if result.returncode != 0:
        if "nothing changed" in result.stdout or "nothing to commit" in result.stdout:
            ...
            return current_commit(scm, source_dir)
        raise RuntimeError(f"Commit failed: {result.stderr}")

    new_hash = current_commit(scm, source_dir)
    _LG.info("Committed %s: %s", new_hash[:12], message[:80])
    return new_hash


def goto(scm: str, source_dir: str, target: str, anchor: str) -> None:
...
def has_pending_changes(scm: str, source_dir: str) -> bool:
