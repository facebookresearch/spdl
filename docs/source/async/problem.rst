Implementing Orchestration System
=================================

One of the problem the async I/O solves is the orchestration of asynchronous
task execution.
In this section, we look at how awkward it is to implement a pipeline system
with a bare thread/process pool executor.
We will see that async I/O's event loop solves the awkwardness and
the async I/O broadly provides mechanism to build complex asynchronous
task chains in a way similar to the regular synchronous, procedual programming.
...
(Python 3.14 will introduce `InterpreterPoolExecutor <https://docs.python.org/3.14/library/concurrent.futures.html#concurrent.futures.InterpreterPoolExecutor>`_.)

There are different constraints when using multi-threading and multi-processing.
Here, we just use multi-threading for an illustration purpose.
