...
A similar analogy applies in performance computing in distributed systems.
...
you can typically see them blocked at ``nccl:all_reduce``.
...
If the bottleneck of the slow rank is data loading (which is almost always
...
Ranks 0 and 19 clearly shows different trands than the rest.
...
queue.
Therefore, the data readiness of the other ranks can stay near 1,
while the data readiness of the slowest node stays at 0,
because the data is immediately consumed.

Now let's try to locate the bottleneck.
This pipeline is composed of download, decompress, and (batch) preprocessing
...
We take an example from a production pipeline consisting of 3 machines,
...
Before 7 AM, the average download time is around 2-3 seconds.
After 7 AM, all ranks experience increased downloading time with higher
variance.

By checking the logs on the storage system, we verified that
this was caused by throttling.
