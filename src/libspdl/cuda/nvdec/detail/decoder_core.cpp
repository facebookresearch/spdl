...
CUvideoparserPtr get_parser(
  ...
  static const auto cb_vseq = [](void* p, CUVIDEOFORMAT* data) -> int {
    return ((NvDecDecoderCore2*)p)->handle_video_sequence(data);
  };
  static const auto cb_decode = [](void* p, CUVIDPICPARAMS* data) -> int {
    return ((NvDecDecoderCore2*)p)->handle_decode_picture(data);
  };
  static const auto cb_disp = [](void* p, CUVIDPARSERDISPINFO* data) -> int {
    return ((NvDecDecoderCore2*)p)->handle_display_picture(data);
  };
  static const auto cb_op = [](void* p, CUVIDOPERATINGPOINTINFO* data) -> int {
    return ((NvDecDecoderCore2*)p)->handle_operating_point(data);
  };
  static const auto cb_sei = [](void* p, CUVIDSEIMESSAGEINFO* data) -> int {
    return ((NvDecDecoderCore2*)p)->handle_sei_msg(data);
  };
...
int NvDecDecoderCore2::handle_video_sequence(CUVIDEOFORMAT* video_fmt) {
...
int NvDecDecoderCore2::handle_decode_picture(CUVIDPICPARAMS* pic_params) {
...
int NvDecDecoderCore2::handle_display_picture(CUVIDPARSERDISPINFO* disp_info) {
...
int NvDecDecoderCore2::handle_operating_point(CUVIDOPERATINGPOINTINFO* data) {
  // Return values:
  // * <0: fail
  // * >=0: succeess
...
int NvDecDecoderCore2::handle_sei_msg(CUVIDSEIMESSAGEINFO* msg_info) {
  // Return values:
  // * 0: fail
  // * >=1: succeeded
...
void NvDecDecoderCore2::decode_packet(
    const uint8_t* data,
    const uint size,
    int64_t pts,
    unsigned long flags) {
...
void NvDecDecoderCore2::decode_packets(
...
void NvDecDecoderCore2::flush(std::vector<CUDABuffer>* buffer) {
  this->frame_buffer = buffer;
  const unsigned char data{};
  decode_packet(&data, 0, 0, CUVID_PKT_ENDOFSTREAM);
}

void NvDecDecoderCore2::reset() {
  if (parser) {
    cb_disabled = true;
    flush(nullptr);
    cb_disabled = false;
