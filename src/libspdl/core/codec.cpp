...
Codec<media>& Codec<media>::operator=(Codec<media>&& other) noexcept {
  using std::swap;
  swap(codecpar_, other.codecpar_);
...
std::string Codec<media>::get_name() const {
  return std::string(avcodec_get_name(codecpar_->codec_id));
}

template <MediaType media>
int64_t Codec<media>::get_bit_rate() const {
  return codecpar_->bit_rate;
}

template <MediaType media>
int Codec<media>::get_bits_per_sample() const {
  return codecpar_->bits_per_raw_sample;
}
...
int Codec<media>::get_num_channels() const
  requires(media == MediaType::Audio)
{
  return GET_NUM_CHANNELS(codecpar_);
}
...
std::string Codec<media>::get_channel_layout() const
  requires(media == MediaType::Audio)
{
  return GET_CHANNEL_LAYOUT_STRING(codecpar_);
}
...
std::string Codec<media>::get_sample_fmt() const
  requires(media == MediaType::Audio)
{
  const char* p = av_get_sample_fmt_name((AVSampleFormat)(codecpar_->format));
  if (p) {
    return std::string(p);
  }
  return "none";
    ...
    default:
      SPDL_FAIL(
          fmt::format(
              ...
              avcodec_get_name(codecpar_->codec_id)));
  }
}
...
std::string Codec<media>::get_pix_fmt() const
  requires(media == MediaType::Video || media == MediaType::Image)
{
  return av_get_pix_fmt_name((AVPixelFormat)(codecpar_->format));
}
...
Rational Codec<media>::get_sample_aspect_ratio() const
  requires(media == MediaType::Video || media == MediaType::Image)
{
  return codecpar_->sample_aspect_ratio;
}
