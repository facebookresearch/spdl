...
FilterGraphImpl::FilterGraphImpl(const std::string& filter_desc)
    : filter_graph_(make_graph(filter_desc)) {
  auto* g = filter_graph_.get();
...
void FilterGraphImpl::add_frames(
  ...
  auto* filter_ctx = inputs_.at(name);
  ...
  if (inputs_.size() != 1) {
    SPDL_FAIL(
        fmt::format(
            ...
            fmt::join(std::views::keys(inputs_), ", ")));
  }
  auto* filter_ctx = inputs_.cbegin()->second;
  for (auto* frame : frames) {
    add_frame(filter_ctx, frame);
  }
}
...
std::string FilterGraphImpl::dump() const {
  char* d = avfilter_graph_dump(filter_graph_.get(), nullptr);
  std::string repr{d};
  av_free(d);
  return repr;
}
...
AnyFrames FilterGraphImpl::get_frames(const std::string& name) {
  auto* filter_ctx = outputs_.at(name);
  ...
  if (outputs_.size() != 1) {
    SPDL_FAIL(
        fmt::format(
            "Key must be provided when there are multiple inputs. "
            "Available values are {}",
            fmt::join(std::views::keys(outputs_), ", ")));
  }
  return this->get_frames(outputs_.cbegin()->first);
}
...
Rational FilterGraphImpl::get_src_time_base() const {
  ...
  return inputs_.cbegin()->second->outputs[0]->time_base;
}
...
Rational FilterGraphImpl::get_sink_time_base() const {
  ...
  return outputs_.cbegin()->second->inputs[0]->time_base;
}
...
#define TS(OBJ, BASE) (static_cast<double>(OBJ->pts) * BASE.num / BASE.den)
  ...
  add_frame(inputs_.cbegin()->second, frame);

  int errnum;
  do {
    AVFramePtr ret = AVFramePtr{CHECK_AVALLOCATE(av_frame_alloc())};
    switch ((errnum = get_frame(outputs_.cbegin()->second, ret.get()))) {
      case AVERROR(EAGAIN):
        co_return;
      case AVERROR_EOF:
        co_return;
