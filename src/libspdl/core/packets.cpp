...
#define NEG_INF -std::numeric_limits<double>::infinity()
  ...
  auto [start, end] =
      packets.timestamp.value_or(std::tuple<double, double>{NEG_INF, POS_INF});
  ...
  for (const auto& pkt : pkts) {
    ...
    ret.emplace_back(ts * modifier);
  }
  std::sort(ret.begin(), ret.end());
  return ret;
}

template std::vector<double> get_timestamps(const AudioPackets&, double);
template std::vector<double> get_timestamps(const VideoPackets&, double);
template std::vector<double> get_timestamps(const ImagePackets&, double);
