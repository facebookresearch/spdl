...
Valid best practice tags: `mtp`, `mp_region`, `batch_size_tuning`, `concurrency_tuning`, `gpu_video_decode`
...
1. **`mtp`** — Apply the **full MTP pattern** from the knowledge base. This is a high-impact optimization, especially for large payloads such as video, and includes continuous mode, subprocess isolation, and proper GPU transfer. **Follow the MTP section in the knowledge base.** Key points:
   ...
   - Do not introduce an `mp_context` override. Omit it to use SPDL's default, or preserve the baseline's explicit choice if it already has one.
   - Build the dataloader once before the epoch loop; use `dl.get_iterator(timeout=...)` for each epoch.

2. **`mp_region`** — Always benchmark region-based multiprocessing with `PipelineBuilder.to(ProcessPoolExecutorConfig(...))`, regardless of data modality. This is a separate architecture from MTP and is mandatory to try for text, images, video, audio, and other workloads. It is especially promising for small text or image records where process-boundary costs are low enough for process parallelism to win.
   - Open the region with `.to(ProcessPoolExecutorConfig(max_workers=N))` and close it with `.to(MAIN_PROCESS)` before `.add_sink()`. Do not introduce an `mp_context` override; preserve a baseline choice if one already exists, otherwise use the default.
   - Put adjacent async I/O and CPU processing stages in one region. Each worker runs a nested Pipeline with its own async event loop; pass async callables directly and do NOT wrap them in `asyncio.run()`.
   - Do NOT assign a `ProcessPoolExecutor` stage by stage. That round-trips intermediate values through IPC and loses the worker-local software pipeline.
   - Prefer aggregating small keys or metadata before the region when the existing operations can consume batches. Keep large fetched/decoded intermediate values inside the worker and return only finished batches.
   - Only region-boundary values cross processes, but those values and all region callables must be picklable. Intermediate values between region stages need not be picklable.
   - Stage concurrency applies inside every worker, so total concurrency is approximately `max_workers * stage_concurrency`. Tune `max_workers` and per-stage concurrency together within the CPU budget.
   - For small boundary values, tune `buffer_size` independently on the opening and closing `.to()` markers to amortize fixed IPC overhead. This does not change the items seen by stages or the training batch size.
   - Keep GPU operations outside the region. For GPU training, compose the region config with `run_pipeline_in_subprocess()` so source iteration and dispatch also stay out of the GPU-driving process, then use a thin main-process frontend for GPU transfer.
   - For large video payloads, retain MTP as the first candidate but still run the MP-region benchmark. When practical, make the region receive small metadata, perform fetch/demux/decode internally, and return a compact or shared-memory-backed result.
   - Write a precise `description` such as: "In the existing builder, aggregate small source metadata before a `.to(ProcessPoolExecutorConfig(max_workers=N))` region; move the adjacent fetch/parse/transform/collate stages into that region without changing their operations; close it with `.to(MAIN_PROCESS)` before the sink; compose the config with `run_pipeline_in_subprocess()` for GPU training; preserve any baseline `mp_context` choice but do not add one; do not use per-stage process executors."

3. **`batch_size_tuning`** — If GPU memory utilization is below 80%, try increasing batch size. Try at least 2 different values.

4. **`concurrency_tuning`** — Adjust concurrency of the bottleneck stage. Try at least 2 different values.
...
5. **`gpu_video_decode`** — If the pipeline decodes video using CPU FFmpeg (`decode_packets` / `decode_packets_ffmpeg` / FFmpeg-based decode functions) and the decode stage is a bottleneck, try replacing it with GPU video decoding via NVDEC (`spdl.io.decode_packets_nvdec`). This offloads decode to dedicated GPU hardware and eliminates the CPU decode bottleneck. **Follow the GPU Video Decoding section in the knowledge base.** Key points:
   ...
   - **Compatible with MTP and MP regions** — `VideoPackets` are picklable. Keep fetch/demux in the subprocess or process region and decode with NVDEC in the main-process frontend. The region is especially useful when it receives small metadata and keeps downloaded bytes inside its workers. NVDEC must remain outside the process boundary because it needs the training process's CUDA context.
   ...
   - Write the `description` field with explicit instructions, e.g.: "Replace `spdl.io.decode_packets(packets)` + `convert_frames` + `transfer_buffer` with `spdl.io.decode_packets_nvdec(packets, device_config=cuda_cfg, pix_fmt='rgb')`. Remove the later `transfer_buffer`/`transfer_tensor` stage since data is already on GPU. Set decode concurrency to 7. Use a dedicated `ThreadPoolExecutor(7)` for the decode stage via `executor=ThreadPoolExecutor(7)`. Create `cuda_cfg` using `spdl.io.cuda_config(device_index, allocator=(torch.cuda.caching_allocator_alloc, torch.cuda.caching_allocator_delete))`. If MTP or an MP region is present, leave fetch/demux there and put NVDEC in the main-process frontend."
...
1. **If best practices remain untried**: Propose experiments that cover them. Prioritize structural changes (MTP and MP region) over parameter tuning, and compare both process topologies rather than assuming one is universally better.
   ...
   **MP-region retry logic**: If an `mp_region` experiment failed at startup,
   preserve the region architecture while repairing pickling, module-level
   callable placement, worker imports, or the pool initializer. Do not turn the
   retry into MTP or per-stage process delegation. Inspect both `.to()`
   boundaries and keep `.add_sink()` after `.to(MAIN_PROCESS)`. Do not mark
   `mp_region` as tried until it succeeds or the retry budget is exhausted.
...
- **`changes`** is the experiment's identity for dedup. List every modification as a short, canonical, snake_case identifier. For code changes use identifiers like `"torch_compile"`, `"fused_adamw"`, `"mtp"`, `"mp_region"`, `"priority_executor"`, `"cache_dataloader"`. For parameter changes use `"param_name=value"` format like `"batch_size=48"`, `"num_workers=16"`. Combination experiments list all changes (e.g. `["mp_region", "torch_compile"]`). Parameter changes are also auto-detected from launch command diffs, but code changes **must** be listed explicitly. The orchestrator rejects experiments whose change set matches a non-failed existing node.
...
- **`goto`** (per-experiment): The orchestrator checks out the instrumentation (anchor) commit before applying each experiment's code changes by default. This ensures every experiment starts from a clean slate. Set `goto` to `null` in most cases. Only set it to a specific commit hash if you want to stack changes on top of a previous successful experiment (e.g., adding batch size tuning on top of an MTP or MP-region experiment that already improved metrics). **The `goto` field also determines the experiment's parent in the hypothesis tree**: `null` means the experiment branches from baseline; a specific commit means it branches from the experiment that produced that commit. This allows a single planning round to propose experiments with different parents (e.g., NVDEC from baseline + batch size from a successful MP-region experiment).
