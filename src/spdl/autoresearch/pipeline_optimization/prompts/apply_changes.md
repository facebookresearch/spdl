...
12. **MP execution-region robustness**: When applying an `mp_region` change, use `PipelineBuilder.to(ProcessPoolExecutorConfig(...))` and close the region with `.to(MAIN_PROCESS)` before `.add_sink()`.
   ...
   - Do NOT attach a `ProcessPoolExecutor` separately to every `.pipe()`. That incurs IPC for every intermediate value instead of keeping the stage sequence inside one worker.
   - Prefer aggregating small source records or metadata before entering the region when the existing stage APIs can consume batches. Only boundary values cross processes; large downloaded/decoded intermediates should be created and consumed inside the region where practical.
   - Region stage callables and boundary inputs/outputs must be picklable. Intermediate values between stages inside the region do not need to be picklable.
   - Stage concurrency is per worker: effective concurrency is approximately `max_workers * stage_concurrency`. Keep that product within the CPU budget.
   - For small boundary values, set `buffer_size` on the opening and closing `.to()` markers to amortize IPC. This transfer buffer is independent of `.aggregate()` and training batch size.
...
14. **TorchTNT scripts**: If the code uses TorchTNT (`torchtnt.framework.fit`, `train`, `AutoUnit`), the SPDL `Pipeline` is passed directly to TorchTNT as the `train_dataloader` (Pipeline is iterable — no wrapper class is required unless instrumentation was added). When applying changes:
