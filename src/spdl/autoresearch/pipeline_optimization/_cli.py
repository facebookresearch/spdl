...
``_run.py`` remains the non-interactive engine implementation invoked by the
...
def _build_engine_command(
    ...
    if ns.engine_command:
        ...
        command = [sys.executable, str(_SCRIPT_DIR / "_run.py")]
...
def main() -> None:
    """Launch the interactive autoresearch supervisor CLI.

    Parses command-line arguments, resolves the supervisor agent, builds
    the engine command, and replaces the current process with the
    interactive supervisor session via :py:func:`os.execvp`.
    """
    _run()
