    ...
    *,
    ...
    Args:
        ...
        fuse_subprocess_stages: If ``True``, fuse runs of two or more adjacent pipe stages that
            ...
            keeps its main-process batching) and splits them into separate runs. Has no effect on
...
def run_pipeline_in_subprocess(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        fuse_subprocess_stages: If ``True``, fuse runs of consecutive pipe stages that share the
            same process-pool (or interpreter-pool) executor instance into a single stage that
            runs the run as one nested pipeline inside a worker pool. The worker processes are
            spawned in (and owned by) the main process, exactly like a hoisted
    ...
    try:
        ...
        raise
    # The hoisted ``ProcessPoolExecutor`` workers and the fused-stage worker pools are all owned
    # by this (the main) process and must outlive every iteration (they are reused across
    # epochs). Wrap the iterable so they are reaped via the wrapper's finalizer when it is torn
    # down — after the epoch loop, not at the end of each iteration. Both kinds share queues with
    # the pipeline subprocess, so the wrapper joins that subprocess before reaping them.
