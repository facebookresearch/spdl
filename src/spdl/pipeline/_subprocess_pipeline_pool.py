...
class _SubprocessPipelinePool:
    ...
    def shutdown(self) -> None:
        ...
        for _ in self._procs:
            try:
                self._in_q.put((_POOL_SHUTDOWN, None))
            except Exception:
                # A failed sentinel for one worker should not block the others; fall through to
                # the join/terminate/kill escalation below.
                continue
        for p in self._procs:
            p.join(3)
