...
std::map<std::string, std::tuple<int, int>> parse_zip(
  ...
  std::map<std::string, std::tuple<int, int>> ret;
  for (uint16_t i = 0; i < eocd.num_entries; ++i) {
    ...
    uint64_t file_size = [&]() -> uint64_t {
      ...
      auto cdhf_end = cd_offset + cdfh.size;
      auto zip64meta = parse_zip64_extended_info(root + cdhf_end);
      return zip64meta.compressed_size;
    }();
