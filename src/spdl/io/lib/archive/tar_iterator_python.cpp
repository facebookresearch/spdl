...
void register_tar(nb::module_& m) {
  ...
  nb::class_<FileObjectTarParser>(m, "FileObjectTarParser")
      .def(
          ...
          [](FileObjectTarParser& v) {
            // Note on the GIL
            // begin constructs FileObjTarParserImpl, which initialize a
            // `nb::tuple` object. So we cannot release the GIL.
            //
            // There is only a little space for releasing the GIL.
            return nb::make_iterator(
  ...
  m.def(
      "parse_tar",
      [](nb::object fileobj) { return FileObjectTarParser{fileobj}; },
      nb::call_guard<nb::gil_scoped_release>());
}
