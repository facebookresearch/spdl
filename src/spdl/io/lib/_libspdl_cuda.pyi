...
def make_nvdec_decoder() -> NvDecDecoder: ...

@overload
def decode_image_nvjpeg(data: memoryview, *, device_config: CUDAConfig, scale_width: int = -1, scale_height: int = -1, pix_fmt: str = 'rgb', sync: bool = True) -> CUDABuffer: ...

@overload
def decode_image_nvjpeg(data: Sequence[memoryview], *, device_config: CUDAConfig, scale_width: int, scale_height: int, pix_fmt: str = 'rgb', sync: bool = True) -> CUDABuffer: ...
...
def transfer_buffer_cpu(buffer: Annotated[NDArray, dict(order='C', device='cuda')]) -> spdl.io.lib._libspdl.CPUBuffer: ...
