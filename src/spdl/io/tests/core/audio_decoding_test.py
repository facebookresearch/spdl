...
from spdl.io.tests.fixture import FFMPEG_CLI, get_sample, load_ref_audio
...
class TestLoadAudio(unittest.TestCase):
    ...
    def test_load_audio(self, sample_fmt: str) -> None:
        # fmt: off
        cmd = f"""
...
class TestBatchAudioConversion(unittest.TestCase):
    def test_batch_audio_conversion(self) -> None:
        # fmt: off
        cmd = f"""
        {FFMPEG_CLI} -hide_banner -y \
        -f lavfi -i sine=sample_rate=8000:frequency=305:duration=5 \
        ...
        hyp = spdl.io.to_numpy(buffer)
        self.assertEqual(hyp.shape, (3, 2, 8000))
