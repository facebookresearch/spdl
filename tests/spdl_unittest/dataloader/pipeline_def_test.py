...
from spdl.pipeline._build import _build_pipeline
from spdl.pipeline._defs import (
    _SinkConfig,
    _SourceConfig,
    Aggregate,
    Disaggregate,
...
def test_source_repr():
    ...
    src = _SourceConfig(list(range(10000)))
...
def test_pipe_args_repr():
    ...
    pipe = Pipe(lst)
    assert len(repr(pipe.args.op)) < 10000
