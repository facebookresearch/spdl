...
def test_decode_video_nvdec(get_sample):
    ...
    async def _test():
        ...
        for frames in results:
            print(frames)
            conversion_tasks.append(spdl.async_convert_frames(frames))
...
async def _decode_image(path):
    ...
    print(packets)
    frames = await spdl.async_decode_packets_nvdec(
...
def test_decode_image_nvdec(get_sample):
    ...
    async def _test():
        ...
        buffer = await spdl.async_convert_frames(frames)
        tensor = spdl.to_torch(buffer)
        print(f"{tensor.shape=}, {tensor.dtype=}, {tensor.device=}")
    ...
    async def _test():
        ...
        for i, result in enumerate(
            ...
            if i == 0:
                ...
                continue
            coro = spdl.async_decode_packets_nvdec(
...
def test_convert_cpu_video_fail(get_sample):
    ...
    async def _test():
        ...
        with pytest.raises(TypeError):
            await spdl.async_convert_frames_cpu(frames)
